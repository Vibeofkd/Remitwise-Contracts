...
impl Orchestrator {
    ...
    /// Validate contract addresses to prevent self-referential or duplicate calls
    ///
    /// This function validates that:
    /// - No address points to the orchestrator itself
    /// - All addresses are distinct
    ///
    /// # Arguments
    /// * `env` - The contract environment
    /// * `addresses` - Variable list of addresses to validate
    ///
    /// # Returns
    /// Ok(()) if all addresses are valid, Err(OrchestratorError::InvalidContractAddress) otherwise
    fn validate_addresses(env: &Env, addresses: &[&Address]) -> Result<(), OrchestratorError> {
    ...
    /// Validate remittance flow addresses
